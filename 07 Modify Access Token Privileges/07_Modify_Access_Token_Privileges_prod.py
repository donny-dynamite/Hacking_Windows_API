"""
(prod) Modify Privileges of an Access Token
- enumerates all privileges listed in the access token for a given process
- enables flipping (enabled <-> disabled) of a given privilege
...
- AdjustTokenPrivileges()   -> modify privilege
"""
...
def get_privilege_selection(buf: ctypes.Structure) -> tuple[int, ctypes.Structure]:
    """ Return int/index of privilege setting to change """

    array_length = buf.PrivilegeCount
    while True:
        try:
            i = int(input(f"\nEnter index of Privilege to flip status [0-{array_length -1}]: "))
...
close_handle(handle_access_token, "AccessToken")
